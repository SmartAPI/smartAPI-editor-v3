...
import getSmartapiConformance from "../../plugins/editor-autosuggest-oas3-keywords/smartapi-conformance"
import getSmartapiConformancePerType from "../../plugins/editor-autosuggest-oas3-keywords/smartapi-conformance-types"
...
export default class Topbar extends React.Component {
  ...
  getopts = () => {
    ...
    let opts = []
    let cnf = getSmartapiConformancePerType()
	cnf.optional.map(function(opt, i){

        let optPath = []
        opt.split(".").map(p => optPath.push(p.charAt(0).toLowerCase() + p.slice(1)));
        let gp = optPath[0]
        let parentKey = optPath[1]
        let childKey = optPath[2]
        let jsv = null

		if (jsContent){
        	if (childKey) {
        		 jsv = jsContent[gp][parentKey]
        	}
        	else  {
        		 childKey = parentKey
        		 parentKey = gp
        		 jsv = jsContent[parentKey]
        	}

        if (jsv) {
              childKey  in jsv ? console.log('key exists') : opts.push(opt)
        	}
        else { opts.push(opt)
        	}
		}
 ...
 getrecoms = () => {
    ...
    let recoms = []
    let cnf = getSmartapiConformancePerType()
	cnf.recommended.map(function(rec, i){
        let recPath = []
        rec.split(".").map(p => recPath.push(p.charAt(0).toLowerCase() + p.slice(1)));
        let gp = recPath[0]
        let parentKey = recPath[1]
        let childKey = recPath[2]
        let jsv = null
        ...
        if (jsv) {
              childKey  in jsv ? console.log('key exists') : recoms.push(rec)
        	}
        else { recoms.push(rec)
        	}
		}
     })
  ...
  handleAddToSpecSubmit = formSubmitEvent => {

    formSubmitEvent.preventDefault();
    let cnt = this.props.specSelectors.specStr()
    let jsonCnt = YAML.safeLoad(cnt)
    for (const checkbox of this.selectedCheckboxes) {

      let path= [];
      checkbox.split(".").map(p => path.push(p.charAt(0).toLowerCase() + p.slice(1)));
      if (path[0] != "info"){
        path[2] = path [1]
        path[1] = path[0]
        path[0] = "info"
      ...
      if(path.length>2){
      	jsonCnt[path[0]][path[1]][path[2]] = ''}

      else {
      	jsonCnt[path[0]][path[1]] = ''
      	}

      let yamlCnt = YAML.safeDump(jsonCnt)

      this.props.specActions.updateSpec(yamlCnt)
  ...
  createCheckboxes = suggestions => (
    suggestions.map(this.createCheckbox)

  )

/// End of smartAPI menu actions
      ...
      <div>
        <div className="topbar">
          <div className="topbar-wrapper">
            ...
            <DropdownMenu {...makeMenuOptions("smartAPI")}>
              <li><button type="button" onClick={this.showModal2}>Upgrade Your Spec</button></li>
               <li>
          <a tabindex="-1" href="https://github.com/WebsmartAPI/swagger-editor/wiki" target="_blank">
            <i class="fa fa-align-justify"></i>&nbsp;
            <span>smartAPI Documentation</span>
          </a>
        </li>
        <li>
          <a tabindex="-1" href="https://github.com/SmartAPI/smartAPI-Specification/blob/OpenAPI.next/versions/3.0.0.md" target="_blank">
            <i class="fa fa-align-justify"></i>&nbsp;
            <span>smartAPI Specification</span>
          </a>
        </li>
        <li>
          <a tabindex="-1" href="http://smart-api.info" target="_blank">
            <i class="fa fa-share-square-o"></i>&nbsp;
            <span>smartAPI Website</span>
          </a>
        </li>
        <li class="divider"></li>
        <li>
          <a href="https://github.com/WebsmartAPI/swagger-editor/issues" target="_blank">
        ...
        <Modal className="swagger-ui modal" ref="modal2">
          <div className="container">
            <h2> Upgrade your spec! </h2>

            <form onSubmit={this.handleAddToSpecSubmit}>

             {this.getrecoms().length>0 ?  <h3>Recommended</h3> : <h3> Congrats! All recommended metadata are already in your spec. </h3>}
              {this.createCheckboxes(this.getrecoms())}
             {this.getopts().length>0 ?  <h3>Optional</h3> : <h3> Congrats! All optional metadata are already in your spec. </h3>}
              {this.createCheckboxes(this.getopts())}

              <div className="right">
            <button className="btn cancel" onClick={this.hideModal2}>Cancel</button>
            <button className="btn" type="submit">Add to spec</button>
