...
// return the array
return suggestionFromSchema(keywordMap, parent)
}
...
function suggestionFromSchema(map, parent) {
  const res = toArray(mapValues(map, (val, key) => {
    ...
    var conformance = "Optional"
    getSmartapiConformance().map((conformances) => {

        //alert("testing "+JSON.stringify(conformances))
       // alert("parent "+parent+"."+keyword);

        if (conformances[parent+"."+keyword] != undefined) {

          conformance = conformances[parent+"."+keyword]
         // alert(parent+"."+keyword + "*"+conformance)
      }
      else {
     // alert("no"+parent+"."+keyword);
      }
    })
    return constructAceCompletion(meta, keyword, conformance)
  }))
  return res
}

function constructAceCompletion(meta, keyword, conformance) {
